; (function ($) {
  $.fn.easyUpload = function (opts) {
    var defaults = {
      ...
      selectType: '选择类型',// 上传类型文案
      selectText: '选择文件',// 上传按钮文案
      multi: true,// 是否允许多文件上传
      multiNum: 20,// 多文件上传时允许的有效文件数
      showNote: true,// 是否展示文件上传说明
      note: '提示：最多上传20个文件，超出默认前20个，支持格式为：doc、epud、docx、pdf',// 文件上传说明
      showPreview: true,
      url: '',// 上传文件地址
    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          var $html = '';
          $html = '<div style="align=center" class="easy_upload-container">';
          $html += '<div class="easy_upload-head">';
          $html += '<span class="easy_upload_head_btn1" style="margin-left:0;">' + option.selectType + '</span>';
          $html += '<select id="filekind" name="filekind" class="easy_upload_head_btn1">';
    	  $html += '<option value="1">社会科学</option>';
		  $html += '<option value="2">自然科学</option>';
		  $html += '<option value="3">悬疑推理</option>';
		  $html += '<option value="4">中国古籍</option>';
		  $html += '<option value="5">人物传记</option>';
		  $html += '<option value="6">当代散文</option>';
		  $html += '<option value="7">中国小说</option>';
		  $html += '<option value="8">中国诗歌</option>';
		  $html += '<option value="9">外国小说</option>';
		  $html += '<option value="10">外国诗歌</option>';
		  $html += '</select></div>';
		  $html += '<div class="easy_upload-head" style="margin-top: 10px;">';
          $html += '<span class="easy_upload_head_btn1" style="margin-left:0;">' + option.selectText + '</span>';
          $html += '<input type="file" ';
          $html += option.multi ? 'multiple ' : '';
          $html += 'class="fileInput" data-count="0" style="display:none;" />';
          $html += '<span class="easy_upload_select noselect" style="margin-left:5px;">' + "本地文件 ..." + '</span>';
          $html += '<span id="multiupload" class="easy_upload_head_btn1 noselect">上传</span>';
        ...
        bindHead: function () {
          ...
          $('#multiupload').off('click').click(function(){
        ...
        _uploadFile: function(target) {
          ...
          function controlUp() {
            if (postedNum < allowFiles.length) {
              upFiniehed = false;
              var filekind = $(target).parent().parent().parent().find('#filekind').value();
              upload();
            } else {
              upFiniehed = true;
            }
          ...
          function upload() {
            if (allowNewPost) {
              ...
              _this._resetParam();
              var fd = new FormData();
              fd.append("kind", $("#filekind").find("option:selected").text());

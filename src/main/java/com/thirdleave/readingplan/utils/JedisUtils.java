...
public class JedisUtils {
    ...
    public static Jedis getJedis() {
        return JedisPoolUtils.getInstance().getJedisPool().getResource();
    }
    ...
    public static void returnJedis(Jedis jedis) {
        if (null != jedis && null != JedisPoolUtils.getInstance().getJedisPool()) {
            JedisPoolUtils.getInstance().getJedisPool().returnResource(jedis);
            ;
        }
    }
    ...
    /*
     * 取消对key过期时间的设置 将带生存时间的转换成一个永不过期的key 当移除成功时返回1，key不存在或者移除不成功时返回0
     */
    ...
    /*
     * 删除keys对应的记录，可以是多个key 返回值是被删除的数量
     */
    ...
    /*
     * 对List，set，SortSet 进行排序，如果集合数据较大应避免使用这个方法 返回排序后的结果，默认升序 sort key Desc为降序
     */
    ...
    /*
     * 查找所有匹配模式的键 key的查询表达式 *代表任意多个 ？代表一个
     */
    ...
    /*
     * sinterstore 这个命令类似于 SINTER 命令，但它将结果保存到 destination 集合，而不是简单地返回结果集。 如果 destination
    ...
    /*
     * 这个命令类似于 SUNION 命令，但它将结果保存到 destination 集合，而不是简单地返回结果集。 如果 destination 已经存在，则将其覆盖。
    ...
    /*
     * zrem 移除有序集 key 中的一个或多个成员，不存在的成员将被忽略。当 key 存在但不是有序集类型时，返回一个错误。在 Redis 2.4 版本以前， ZREM
     * 每次只能删除一个元素。
